...
module {
  ...
  func.func @main(%arg0: tensor<6x12xf32>) -> tensor<6x12xf32> {
    ...
    %2 = tensor.empty() : tensor<6x12xf64>
    %3 = linalg.generic {indexing_maps = [#map], iterator_types = ["parallel", "parallel"]} outs(%2 : tensor<6x12xf64>) {
    ^bb0(%out: f64):
      %10 = linalg.index 0 : index
      %11 = arith.index_cast %10 : index to i64
      %12 = linalg.index 1 : index
      %13 = arith.index_cast %12 : index to i64
      %14 = arith.muli %11, %c12_i64 : i64
    ...
    %4 = tensor.empty() : tensor<6x12xi1>
    %5 = linalg.generic {indexing_maps = [#map, #map1, #map], iterator_types = ["parallel", "parallel"]} ins(%3, %cst : tensor<6x12xf64>, tensor<f64>) outs(%4 : tensor<6x12xi1>) {
    ^bb0(%in: f64, %in_3: f64, %out: i1):
      %10 = arith.cmpf olt, %in, %in_3 : f64
      linalg.yield %10 : i1
    } -> tensor<6x12xi1>
    %6 = tensor.empty() : tensor<6x12xf32>
    %7 = linalg.generic {indexing_maps = [#map, #map], iterator_types = ["parallel", "parallel"]} ins(%5 : tensor<6x12xi1>) outs(%6 : tensor<6x12xf32>) {
    ^bb0(%in: i1, %out: f32):
      %10 = arith.uitofp %in : i1 to f32
      linalg.yield %10 : f32
    } -> tensor<6x12xf32>
    %8 = linalg.generic {indexing_maps = [#map, #map, #map], iterator_types = ["parallel", "parallel"]} ins(%7, %arg0 : tensor<6x12xf32>, tensor<6x12xf32>) outs(%6 : tensor<6x12xf32>) {
    ^bb0(%in: f32, %in_3: f32, %out: f32):
      %10 = arith.mulf %in, %in_3 : f32
      linalg.yield %10 : f32
    } -> tensor<6x12xf32>
    %9 = linalg.generic {indexing_maps = [#map, #map], iterator_types = ["parallel", "parallel"]} ins(%8 : tensor<6x12xf32>) outs(%6 : tensor<6x12xf32>) {
    ^bb0(%in: f32, %out: f32):
      %10 = arith.divf %in, %cst_2 : f32
      linalg.yield %10 : f32
    } -> tensor<6x12xf32>
    return %9 : tensor<6x12xf32>
  }
}
